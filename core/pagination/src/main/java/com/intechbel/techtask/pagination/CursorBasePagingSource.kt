...
import com.intechbel.techtask.shared.common_models.PagingKey
import com.intechbel.techtask.shared.common_models.PagingParam

abstract class CursorBasePagingSource<T : PagingKey, Param : PagingParam> : PagingSource<Int, T>() {

    val paging: MutableStateFlow<PagingData<T>> = MutableStateFlow(PagingData.empty())

    private var pagingParam: Param? = null

    operator fun invoke(pagingParam: Param, coroutineScope: CoroutineScope) = coroutineScope.launch {
        getPagingList(pagingParam).cachedIn(this).collectLatest {
            paging.value = it
        }
    }

    protected abstract fun pagingUseCase(): CursorPagingUseCase<T, Param>

    protected abstract fun newInstance(): CursorBasePagingSource<T, Param>

    private fun <T : PagingKey> CursorBasePagingSource<T, Param>.getPagingList(pagingParam: Param): Flow<PagingData<T>> {
        return Pager(
            config = PagingConfig(
                pageSize = PAGE_SIZE,
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, T> {
        ...
        val data = when (val response = pagingUseCase()(params.key, param)) {
        ...
        return LoadResult.Page(
            data = data,
            prevKey = if (nextPage == 0) null else nextPage,
            nextKey = if (data.isEmpty()) null else data.lastOrNull()?.key
        )
    }
